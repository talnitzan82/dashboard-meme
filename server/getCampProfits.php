...
$qry_em = "SELECT sCampaignID, COUNT(id) AS Requests,SUM(adtagviewed) AS AdImp,(SUM(adtagviewed) / COUNT(id)) * 100 AS fill,sum(sCost)/1000 as cost, SUM(adtagviewed)*0.0035 as revenue, SUM(adtagviewed)*0.0035 - sum(sCost)/1000 as profit, ((SUM(adtagviewed)*0.0035 - sum(sCost)/1000)/(SUM(adtagviewed)*0.0035))*100 as prmargin FROM " . $table . " where tag = '" . $tag . "' GROUP BY sCampaignID;";
//var_dump($qry_em);
